...
Epoll::Epoll() : epfd(-1), events(nullptr){
    epfd = epoll_create1(0);    // 创建一个epoll文件描述符号
    errif(epfd == -1, "epoll创建错误!");  // 初始化
    events = new epoll_event[MAX_EVENTS];  // 事件树
...
std::vector<epoll_event> Epoll::poll(int timeout){
    ...
    int nfds = epoll_wait(epfd, events, MAX_EVENTS, timeout);   // 获取事件
